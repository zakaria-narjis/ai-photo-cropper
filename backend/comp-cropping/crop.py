...
class Cropper2:
    ...
    def process_images(self,images_as_bytes:bytes)->tuple[list,list,list]:
        im_widths = []
        im_heights= []
        resized_images =[]     
        for image in images_as_bytes:
            image = Image.open(BytesIO(image)).convert('RGB')
            resized_image, im_width, im_height = self.process(image)
        ...
        resized_images = torch.tensor(resized_images)
    ...
    def crop_images(self,images):
        im, im_width, im_height, source_img = self.process_images(images)
        if im.dim()<4:
            im = im.unsqueeze(0)
        im = im.to(self.device)
        logits,kcm,crop = self.predict(im)
