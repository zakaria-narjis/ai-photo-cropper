...
from api_handler import ClipCropAPI as api
...
with st.form(key="clip_crop_form"):
    ...
    ss.user.query = st.text_input(
        label="Enter Keyword for Image Crop",
        help="Type a keyword that describes the object or section you want to crop from the image.",
        placeholder="e.g., 'tree', 'logo'",
    )
    submitted = st.form_submit_button(label="Crop")

if submitted:
    if ss.user.image_file is not None and ss.user.query is not None:
